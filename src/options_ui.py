...
class OptionsUI(tk.Toplevel):
    ...
    def _make_layout(self, parent):
        ...
        timer_opts = tk.Frame(parent, bd=self._boarderwidth, relief="groove")
        ...
        timer_opts.grid(row=1, column=2, sticky='nsew', padx=self._padding, pady=self._padding)
    ...
    def _include_numbers_frame(self, parent):
    ...
    def _include_operators_frame(self, parent):
    ...
    def _include_patterns_frame(self, parent):
    ...
    def _timer_options_frame(self, parent):
        ...
        entry = tk.Entry(frame, font=self._small_font, width=1, textvariable=self._timer_vals)
    ...
    def _change_username_frame(self, parent):
    ...
    def _num_of_facts_frame(self, parent):
    ...
    def _reset_password_frame(self, parent):
    ...
    def _delete_user_frame(self, parent):
    ...
    def _main_buttons_frame(self, parent):
    ...
    def _on_reset(self):
        ...
        if messagebox.askyesno("Reset Password", message=message):
            ...
            os.remove(pass_path)
            messagebox.showinfo("Password Reset", "Your password has been reset.")
            self.destroy()
    ...
    def _save_settings(self):
        ...
        self._user.inc_nums = {k: var.get() for k, var in self._inc_nums.items()}
        self._user.inc_oprs = {k: var.get() for k, var in self._inc_oprs.items()}
        self._user.inc_ptrns = {k: var.get() for k, var in self._inc_ptrns.items()}
